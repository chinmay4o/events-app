import React, { useEffect, useState, useRef } from "react";
import EditProfile from "./EditProfile";
import ReferAndEarn from "./ReferAndEarn";
import { logout } from "../../redux/actions/userActions";
import { useDispatch } from "react-redux";
import { USER_EVENTS_EMPTY } from "../../redux/constants/userConstants";
import { EVENT_CREATE_DESTROY } from "../../redux/constants/eventConstants";
import useQRCode from "../../helper/hooks/useQRCode.tsx";
import ScanIcon from "../../common/scanIcon/ScanIcon";
import DefaultProfilePicture from "../../common/defaultProfilePicture/DefaultProfilePicture";
import { useNavigate } from "react-router-dom";

function AccountPanel({ savedUserConfig }) {
  const [showProfile, setShowProfile] = useState(true);
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const qrOptions = {
    width: 150,
    height: 150,
    data: `https://dev.warpbay.com${
      savedUserConfig?._id ? `/connections/user?uid=${savedUserConfig._id}` : ""
    }`,
    margin: 0,
    qrOptions: { typeNumber: "0", mode: "Byte", errorCorrectionLevel: "Q" },
    imageOptions: { hideBackgroundDots: true, imageSize: 0.4, margin: 0 },
    dotsOptions: { type: "extra-rounded", color: "#000000" },
    backgroundOptions: { color: "#ffffff" },
    image:
      "data:image/png;base64,",
    dotsOptionsHelper: {
      colorType: { single: true, gradient: false },
      gradient: {
        linear: true,
        radial: false,
        color1: "#6a1a4c",
        color2: "#6a1a4c",
        rotation: "0",
      },
    },
    cornersSquareOptions: { type: "extra-rounded", color: "#7143c7" },
    cornersSquareOptionsHelper: {
      colorType: { single: true, gradient: false },
      gradient: {
        linear: true,
        radial: false,
        color1: "#000000",
        color2: "#000000",
        rotation: "0",
      },
    },
    cornersDotOptions: { type: "", color: "#7a48ad" },
    cornersDotOptionsHelper: {
      colorType: { single: true, gradient: false },
      gradient: {
        linear: true,
        radial: false,
        color1: "#000000",
        color2: "#000000",
        rotation: "0",
      },
    },
    backgroundOptionsHelper: {
      colorType: { single: true, gradient: false },
      gradient: {
        linear: true,
        radial: false,
        color1: "#ffffff",
        color2: "#ffffff",
        rotation: "0",
      },
    },
  };

  const qrCode = useQRCode(qrOptions);
  const ref = useRef(null);

  useEffect(() => {
    if (!localStorage.getItem("accessToken")) {
      alert("Please Login");
      navigate("/login");
    }
    if (ref.current && savedUserConfig?._id) qrCode?.append(ref.current);
  }, [savedUserConfig?._id]);

  return (
    <>
      {showProfile === true && (
        <div
          className={`max-w-[1440px] px-[16px] w-full mx-auto min-h-[calc(100vh-58px)] md:w-[422px]`}
        >
          <div className="w-full h-[570px] mt-10">
            <div ref={ref} className="grid place-items-center"></div>
            <p className="text-black text-opacity-50 pt-5 text-center">
              {savedUserConfig?.email}
            </p>
            <div className="mt-5">
              <hr />
            </div>

            <div className="flex items-center w-full md:w-[387px] h-[50px] mt-5">
              {savedUserConfig?.profilePicture ? (
                <img
                  src={savedUserConfig.profilePicture}
                  alt="User"
                  className="h-[60px] w-[60px] rounded-[50%] my-auto object-cover"
                />
              ) : (
                <DefaultProfilePicture
                  firstName={savedUserConfig?.firstName}
                  lastName={savedUserConfig?.lastName}
                  style={{
                    height: "64px",
                    width: "64px",
                    borderRadius: "32px",
                  }}
                />
              )}
              <div className="ml-5 w-full md:w-[259px] text-[14px]">
                <p className="font-semibold text-[17px] ">{`${savedUserConfig?.firstName} ${savedUserConfig?.lastName}`}</p>
                <p className="text-black text-opacity-50 mt-[0px]">
                  {savedUserConfig?.organization}
                </p>
              </div>
              <img
                src="/svgs/Edit.svg"
                alt="edit"
                className="h-[23px] w-[23px] md:h-[18px] md:w-[18px] cursor-pointer relative -left-[25px] md:static"
                onClick={() => setShowProfile("editProfile")}
              />
            </div>
            <div className="mt-5">
              <hr />
            </div>
            {/* 
            <div
              className="flex items-center w-full md:w-[387px] h-[50px] mt-5 cursor-pointer"
              onClick={() => setShowProfile("referAndEarn")}
            >
              <div className="h-12 w-12 rounded-[60px] my-auto bg-[#F8FFAA]"></div>
              <div className="ml-5 w-full md:w-[259px] text-[15px]">
                <p className="font-semibold text-[14px] ">Refer & Earn</p>
                <p className="text-black text-opacity-50 mt-[0px] text-[13px]">
                  240 points
                </p>
              </div>
              <img src="/svgs/right.svg" alt="edit" className="h-[18px] w-[18px]" />
            </div>

            <div className="mt-5">
              <hr />
            </div> */}

            {/* <div className="flex items-center w-full md:w-[387px] h-[50px] mt-5 cursor-pointer">
              <div className="h-16 w-16 rounded-[60px] my-auto bg-[#BEE8FF]"></div>
              <div className="ml-5 w-full md:w-[259px] text-[14px]">
                <p className="font-semibold text-[17px] ">My Schedule</p>
                <p className="text-black text-opacity-50 mt-[0px]">
                  Next meeting: 23 Nov, 2022
                </p>
              </div>
              <img src="/svgs/right.svg" alt="edit" className="h-[18px] w-[18px]" />
            </div>

            <div className="mt-5">
              <hr />
            </div> */}

            {/* <div className="flex items-center w-full md:w-[387px] h-[50px] mt-5 cursor-pointer">
              <div className="h-16 w-16 rounded-[60px] my-auto bg-[#FFCFBF]"></div>
              <div className="ml-5 w-full md:w-[259px] text-[14px]">
                <p className="font-semibold text-[17px] ">
                  Manage Subscription
                </p>
                <p className="text-black text-opacity-50 mt-[0px]">
                  You’re on the free plan now
                </p>
              </div>
              <img src="/svgs/right.svg" alt="edit" className="h-[18px] w-[18px]" />
            </div> */}
            {/* <div className="mt-5">
              <hr />
            </div> */}
            {/* <div className="mx-auto my-0"> */}
            <div
              className={`cursor-pointer w-[140px] bg-danger text-[16px] grid place-items-center h-[40px] rounded-[8px] text-[#fff] font-[600] mt-[30px] mx-auto`}
              onClick={() => {
                let linkedinAccessToken = localStorage.getItem(
                  "linkedinAccessToken"
                );
                localStorage.clear();
                localStorage.setItem(
                  "linkedinAccessToken",
                  linkedinAccessToken
                );
                dispatch(logout());
                dispatch({
                  type: USER_EVENTS_EMPTY,
                  payload: [],
                });
                dispatch({
                  type: EVENT_CREATE_DESTROY,
                });
                navigate("/login");
              }}
            >
              Logout
            </div>
            {/* </div> */}

            {/* <p className="text-[24px] font-[700] block text-left w-full">Profile</p>
                    <div className="m-3">
                        <p className="text font-bold">first Name</p>
                        <p>{savedUserConfig?.firstName}</p>
                    </div>

                    <div className="m-3">
                        <p className="text font-bold">Last Name</p>
                        <p>{savedUserConfig?.lastName}</p>
                    </div>
                    <div className="m-3">
                        <p className="text font-bold">Email</p>
                        <p>{savedUserConfig?.email}</p>
                    </div>
                    <div className="m-3">
                        <p className="text font-bold">Organization</p>
                        <p>{savedUserConfig?.organization}</p>
                    </div> */}
          </div>
        </div>
      )}
      <ScanIcon />

      {showProfile === "editProfile" && (
        <EditProfile
          setShowProfile={setShowProfile}
          savedUserConfig={savedUserConfig}
        />
      )}
      {showProfile === "referAndEarn" && (
        <ReferAndEarn setShowProfile={setShowProfile} />
      )}
    </>
  );
}

export default AccountPanel;
